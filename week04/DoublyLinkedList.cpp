...
bool areListsEqual(List *list1, List *list2)
{
    ...
    current1 = list1 ? list1->p_tail : nullptr;
    current2 = list2 ? list2->p_tail : nullptr;
    while (current1 && current2)
    {
        if (current1->key != current2->key)
        {
        ...
        current1 = current1->p_prev;
        current2 = current2->p_prev;
    }
...
bool addHead(List *&L, int data)
{
    ...
    if (n != nullptr) {
        if (L->p_head == nullptr)
            ...
            n->p_next = L->p_head, L->p_head->p_prev = n, L->p_head = n;
        return true;
    }
...
bool addPos(List *&L, int data, int pos)
{
    ...
    if (k == nullptr)
        return false;
    if (k == L->p_tail)
        L->p_tail->p_next = l, l->p_prev = L->p_tail, L->p_tail = l;
    else
        l->p_next = k->p_next, l->p_prev = k, k->p_next = l, l->p_next->p_prev = l;
...
int main()
{
    ...
    assert(node1 != nullptr && node1->key == 10 && node1->p_next == nullptr && node1->p_prev == nullptr);
    ...
    assert(list2->p_head->p_next == nullptr && list2->p_head->p_prev == nullptr);
    std::cout << "Passed" << std::endl;
    freeList(list2);
    ...
    assert(list3->p_head->p_next == nullptr && list3->p_head->p_prev == nullptr);
    assert(addHead(list3, 40));
    assert(list3->p_head->key == 40 && list3->p_head->p_next->key == 30 && list3->p_tail->key == 30);
    assert(list3->p_head->p_prev == nullptr && list3->p_head->p_next->p_prev == list3->p_head);
    std::cout << "Passed" << std::endl;
    freeList(list3);
    ...
    assert(list4->p_head->p_next == nullptr && list4->p_head->p_prev == nullptr);
    assert(addTail(list4, 60));
    assert(list4->p_head->key == 50 && list4->p_tail->key == 60 && list4->p_head->p_next->key == 60);
    assert(list4->p_tail->p_prev->key == 50 && list4->p_tail->p_next == nullptr);
    std::cout << "Passed" << std::endl;
    freeList(list4);
    ...
    assert(removeHead(list5) && list5->p_head->key == 80 && list5->p_head->p_prev == nullptr);
    assert(removeHead(list5) && list5->p_head->key == 90 && list5->p_head->p_prev == nullptr);
    ...
    assert(list6->p_tail->p_prev->key == 100);
    removeTail(list6);
    ...
    assert(list10->p_head->p_next == nullptr && list10->p_head->p_prev == nullptr);
    assert(addPos(list10, 290, 1));
    assert(list10->p_head->key == 280 && list10->p_tail->key == 290 && list10->p_head->p_next->key == 290);
    assert(list10->p_tail->p_prev->key == 280 && list10->p_tail->p_next == nullptr);
...
NODE *pred(List *&L, NODE *p_node) {
    (void)L;
    return p_node ? p_node->p_prev : nullptr;
}

NODE *find_first_of(List *&L, int key) {
    for (NODE *k = L->p_head; k != nullptr; k = succ(L, k))
        if (k->key == key)
            return k;
    return nullptr;
}
...
bool remove_node(List *&L, NODE *p_node) {
    ...
    NODE *k = pred(L, p_node);
    NODE *l = succ(L, p_node);
    if (k)
        k->p_next = l;
    if (l)
        l->p_prev = k;
...
NODE *extract_node(List *&L, int index) {
    NODE *k;
    for (k = L->p_head; k != nullptr && index > 0; k = k->p_next, index--);
