...
namespace sort {
    long long bubble_sort(int *__first, int *__last) {
        ...
        for (int *i = __last; (++comparisons, i != __first); i = new_i) {
            new_i = __first;
            for (int *j = __first; (++comparisons, sort_util::distance(j, i) > 1); ++j)
    ...
    long long selection_sort(int *__first, int *__last) {
        ...
        for (int *i = __first; (++comparisons, i != __last); ++i) {
            int *min_ptr = i;
            for (int *j = i + 1; (++comparisons, j != __last); ++j)
    ...
    long long insertion_sort(int *__first, int *__last) {
        ...
        for (int *i = __first + 1; (++comparisons, i != __last); ++i)
            for (int *j = i; (++comparisons, j != __first); --j) {
    ...
    long long merge_sort_iter(int *__first, int *__last) {
        ...
        for (int D = 1; (++comparisons, D < __len); D *= 2)
            for (int *p = __first, *p_aux = aux; (++comparisons, sort_util::distance(p + D, __last) > 0); p += D * 2, p_aux += D * 2) {
    ...
    long long heap_sort(int *__first, int *__last) {
        ...
        while ((++comparisons, __first != __last))
    ...
    long long quick_sort(int *__first, int *__last) {
        ...
        // Hybrid
        if (++comparisons, sort_util::distance(__first, __last) <= 8)
        ...
        // Tail recursion
        if (++comparisons, sort_util::distance(__first, p) < sort_util::distance(p, __last)) {
    ...
    long long radix_sort(int *__first, int *__last) {
        ...
        for (int _digit_place = 1; ++comparisons, max_value / _digit_place > 0; _digit_place *= 10) {
    ...
    long long shell_sort(int *__first, int *__last) {
        ...
        int gap = 1;
        while ((++comparisons, gap < __len / 3))
            gap = gap * 3 + 1;
        
        while (gap > 0) {
            for (int *i = __first + gap; (++comparisons, i != __last); ++i)
                for (int *j = i; (++comparisons, sort_util::distance(__first, j) >= gap) && (++comparisons, *j < *(j - gap)); j -= gap)
    ...
    long long shaker_sort(int *__first, int *__last) {
        ...
        while (__first < __last) {
            ...
            for (int *i = __first; (++comparisons, sort_util::distance(i, __last) > 1); ++i) {
            ...
            __last = __n_last;
            for (int *i = __last; (++comparisons, sort_util::distance(__first, i) > 1); --i) {
    ...
    long long counting_sort(int *__first, int *__last) {
        ...
        for (int i = 0; (++comparisons, i < _size); i++)
            _v_count[i] = 0;
        ...
        for (int i = 0; (++comparisons, i < _size); i++)
            while ((++comparisons, _v_count[i] > 0))
    ...
    long long flash_sort(int *__first, int *__last) {
        ...
        if ((++comparisons, min_value == max_value))
            return comparisons;
        ...
        auto find_swap_index = [&__bucket_size, &get_bucket_id, &__first, &comparisons](int __bucket_id) -> int {
            int i;
            for (i = __bucket_size[__bucket_id - 1]; (++comparisons, i < __bucket_size[__bucket_id]); ++i)
                if (get_bucket_id(*(__first + i)) != __bucket_id)
                    break;
            return i;
        };

        auto arrange_bucket = [&__first, &get_bucket_id, &find_swap_index, &comparisons](int l, int r, int v) -> void {
            for (int i = l; (++comparisons, i < r); i++) {
                int __bucket_id = get_bucket_id(*(__first + i));
                while ((++comparisons, __bucket_id != v)) {
        ...
        for (int i = 0; i < (++comparisons, m - 1); i++) {
            if ((++comparisons, i == 0))
